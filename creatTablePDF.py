...
def make_pdf_table(weathe_forecast):
    data = [
        ["Time", "Temperature", "Relative Humidity", "Windspeed 180m" ,], # 'testing','size'],
    ]
    def from_json_to_readalble_forecast(weather):
        li = []
        for x in range(len(weather['hourly']['time'])):
            data.append([str(weather['hourly']['time'][x]) , str(weather['hourly']['temperature_2m'][x]) + "°C" , str(weather['hourly']['relativehumidity_2m'][x]) + "%" , str(weather['hourly']['windspeed_180m'][x]) ])
        # return li
    global pdf
    try:
        del pdf
